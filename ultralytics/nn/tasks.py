...
from ultralytics.nn.modules import (
    ...
    SimAM,
    LERFBlock,
    EMA, # Use local EMA from edgs if needed, but tasks.py imports EMA from modules which imports from edgs now
    C3Ghost,
    DCNv2,
    EdgeFlow,
    SignalFocus,
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        # ----------------------MACF and CAM handling----------------------
        elif m is MACF:
            # MACF takes list of inputs [local, global] and fuses them
            # Output channel is specified in args[0], needs width scaling
            c2_raw = args[0] if args else sum(ch[x] for x in f)
            c2 = make_divisible(min(c2_raw, max_channels) * width, 8)  # Apply width scaling
            # Pass input channels to MACF: [ch_local, ch_global]
            ...
            # CAM is single input attention module
            c1 = ch[f]
            c2_raw = args[0] if args else c1  # Output channels, default same as input
            c2 = make_divisible(min(c2_raw, max_channels) * width, 8)  # Apply width scaling
            args = [c1, c2]
